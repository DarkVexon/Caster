...
public class EnchantedTooltips {
    ...
    public static void Postfix(float x, float y, SpriteBatch sb, ArrayList<String> keywords) {
        ...
        try {
            ...
            if (EnchantedCardsPatch.isEnchanted(acard) && !acard.rawDescription.toLowerCase().contains("enchant") && !acard.rawDescription.toLowerCase().contains("enchanted")) {
                tooltips.add(new TooltipInfo("#yEnchanted", "At the start of your turn, a random card in your hand is #yEnchanted for that turn."));
            }
            if (!tooltips.isEmpty())
